...
int main()
{
    ...
    std::string env_data_path = "../data/layout/single/50x50";
    ...
    std::vector<rds::GNNAllocationBufferHandle> train_buffers;
    std::vector<rds::GNNAllocationHandle> train_allocations;
    ...
    for (int id = 0; id < num_train_envs; id++)
    {
        ...
        train_buffers.push_back(std::make_shared<rds::GNNAllocationBuffer>(num_train_task, gamma, lambda, device));
        train_allocations.push_back(std::make_shared<rds::GNNAllocation>(train_envs[id]->swarm(), train_envs[id]->graph_handle(),
                                                                         train_task_gens[id], train_buffers[id], train_envs[id]->map_length(),
                                                                         train_envs[id]->map_width(), device, train_envs[id]->dt(), rds::TRAINING));
        train_planners.push_back(std::make_shared<rds::AStarPlanner>(train_envs[id]->swarm(), train_envs[id]->graph_handle(), train_envs[id]->map_center(),
        ...
        train_figs[id]->save("../visualize/env_" + std::to_string(id) + ".png");
    }
    ...
    rds::GNNAllocationBufferHandle test_buffer = std::make_shared<rds::GNNAllocationBuffer>(num_test_task, gamma, lambda, device);
    ...
    torch::jit::script::Module gnn_allocation_module;
    if (std::filesystem::exists(model_folder + "/model.pth"))
    {
        gnn_allocation_module = torch::jit::load(model_folder + "/model.pth", device);
    }
    else
    {
        gnn_allocation_module = torch::jit::load("../data/original_model/gnn_allocation/gnn_allocation_policy_gpu_ver3.pth", device);
    }

    gnn_allocation_module.train(true);
    std::vector<at::Tensor> parameters;
    for (const auto &params : gnn_allocation_module.parameters())
    {
    ...
    for (int iter = start_iteration; iter <= num_iterations; iter++)
    {
        ...
        while (true)
        {
            ...
            for (int env_id = 0; env_id < num_train_envs; env_id++)
            {
                ...
                rds::GNNAllocationObservation obs = train_allocations[env_id]->observation();
                if (obs.state == true)
                {
                    rds::PolicyOut out = policy.action_sample(gnn_allocation_module, obs.node_features,
                                                              obs.edge_index, obs.edge_features,
                                                              obs.start_ids, obs.target_ids, obs.robot_ids, obs.selected_ids);
                    // std::cout << "--------------------------------" << std::endl;
                    // std::cout << obs.node_features << std::endl;
                    // std::cout << obs.edge_index << std::endl;
                    // std::cout << obs.edge_features << std::endl;
                    train_allocations[env_id]->allocation(obs.robot_id, out.action.item().toInt());
                    if (train_buffers[env_id]->is_full() == false)
                    {
                        train_buffers[env_id]->update(obs.node_features, obs.edge_features, obs.edge_index, obs.start_ids,
                                                      obs.target_ids, obs.robot_ids, obs.selected_ids,
                                                      out.action, out.value, out.log_prob);
                    }
        ...
        // Testing
        {
            ...
            float average_time = fabs(test_buffer->rewards().mean().item().toFloat());
            ...
            if (best_average_time > average_time)
            {
                ...
                save_best_average_time_model(model_folder, gnn_allocation_module, best_average_time);
            ...
            //     save_best_execution_time_model(model_folder, gnn_allocation_module, best_execution_time);
        ...
        rds::GNNAllocationBuffers buffers(train_buffers);
        ...
        for (int epoch = 0; epoch < num_epochs; epoch++)
        {
            ...
            for (int start = 0; start < total_batch_size; start += mini_batch_size)
            {
                ...
                rds::ObservationFromBuffer mb_buffers = rds::get_observation_from_buffer(buffers, mb_inds);
                rds::PolicyOut new_out = policy.action_evaluation(gnn_allocation_module,
                                                                  mb_buffers.node_features, mb_buffers.edge_index,
                                                                  mb_buffers.edge_features, mb_buffers.start_ids,
                                                                  mb_buffers.target_ids, mb_buffers.robot_ids,
                                                                  mb_buffers.selected_ids,
        ...
        save_model(model_folder, gnn_allocation_module, iter);
        for (int env_id = 0; env_id < num_train_envs; env_id++)
        {
            train_buffers[env_id]->clear();
        }
...
//// rds::PolicyOut out = policy.action_sample(gnn_allocation_module, obs.node_features, obs.edge_index, obs.edge_features, obs.start_ids, obs.target_ids);
// torch::Tensor action = out.action.expand({2}).to(device);
// std::cout << out.action << " " << out.value << " " << out.log_prob << " " << out.entropy << std::endl;

// rds::PolicyOut out1 = policy.action_evaluation(gnn_allocation_module, {obs.node_features, obs.node_features},
//                                             {obs.edge_index, obs.edge_index}, {obs.edge_features, obs.edge_features},
//                                             {obs.start_ids, obs.start_ids}, {obs.target_ids, obs.target_ids}, action);
// std::cout << out1.action << " " << out1.value << " " << out1.log_prob << " " << out1.entropy << std::endl;
// torch::jit::script::Module gnn_allocation_module = torch::jit::load("../data/original_model/gnn_allocation/gnn_allocation_policy18.pth", torch::kCUDA);
...
//     parameters.push_back(params);
// }
// torch::jit::script::Module module = torch::jit::load("../data/original_model/gnn_allocation/gnn_allocation_policy_cpu.pth", device);

// int i = 0;
// for (const auto &params : module.parameters())
// {
//     params.set_data(parameters[i].to(device).data());
//     i++;
// }
// c10::IValue out_cpu = module.forward(torch::jit::Stack({obs.node_features,
//                                                         obs.edge_index,
//                                                         obs.edge_features,
//                                                         obs.start_ids,
//                                                         obs.target_ids}));

// c10::IValue out_cuda = gnn_allocation_module.forward(torch::jit::Stack({obs.node_features.to(torch::kCUDA),
//                                                                         obs.edge_index.to(torch::kCUDA),
//                                                                         obs.edge_features.to(torch::kCUDA),
//                                                                         obs.start_ids.to(torch::kCUDA),
//                                                                         obs.target_ids.to(torch::kCUDA)}));

// torch::Tensor probs_cpu = out_cpu.toTuple()->elements()[0].toTensor();
// torch::Tensor value_cpu = out_cpu.toTuple()->elements()[1].toTensor();

// torch::Tensor probs_cuda = out_cuda.toTuple()->elements()[0].toTensor().to(device);
// torch::Tensor value_cuda = out_cuda.toTuple()->elements()[1].toTensor().to(device);

// std::cout << probs_cpu - probs_cuda << std::endl;
// std::cout << value_cpu - value_cuda << std::endl;