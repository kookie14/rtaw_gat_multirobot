...
namespace rds
{
    SingleReconstruction::SingleReconstruction(std::string data_path)
    {
    ...
    void SingleReconstruction::set_data_path(std::string data_path)
    {
        data_path_ = data_path;
        read_map_data();
        build_connected_graph();
    }
    Graph SingleReconstruction::graph() { return graph_; }
    Vertices SingleReconstruction::waiting_vertices() { return wa_vertices_; }
    Point2D SingleReconstruction::map_center() { return map_center_; }
    float SingleReconstruction::map_width() { return map_width_; }
    float SingleReconstruction::map_length() { return map_length_; }
    float SingleReconstruction::single_line_width() { return single_line_width_; }
    float SingleReconstruction::double_line_width() { return double_line_width_; }

    void SingleReconstruction::read_map_data()
    {
    ...
    void SingleReconstruction::build_connected_graph()
    {
    ...
    void SingleReconstruction::build_graph_vertices()
    {
    ...
    void SingleReconstruction::build_graph_edges()
    {
    ...
    void SingleReconstruction::build_graph_vertices_from_zone()
    {
    ...
    void SingleReconstruction::build_graph_vertices_from_single_horizontal_line()
    {
        for (int i = 0; i < s_h_lines_.size(); i++)
        {
    ...
    void SingleReconstruction::build_graph_vertices_from_single_vertical_line()
    {
        for (int i = 0; i < s_v_lines_.size(); i++)
        {
    ...
    void SingleReconstruction::build_graph_vertices_from_intersection_line()
    {
        for (int i = 0; i < s_v_lines_.size(); i++)
        {
    ...
    void SingleReconstruction::build_graph_vertices_from_s_v_and_s_h(Line v_l, int line_idx)
    {
        for (int i = 0; i < s_h_lines_.size(); i++)
        {
    ...
    void SingleReconstruction::build_graph_edges_from_zone_and_lines()
    {
    ...
    void SingleReconstruction::build_graph_edges_in_waiting_zone()
    {
        ...
        for (Vertex v1 : wa_vertices_)
        {
            for (Vertex v2 : wa_vertices_)
            {
                ...
                if (distance > 1.0)
                    continue;
                if (fabs(v1.x() - v2.x()) < SAME_DIST and fabs(v1.y() - v2.y()) > SAME_DIST)
                {
                ...
                // else if (fabs(v1.x() - v2.x()) > SAME_DIST and fabs(v1.y() - v2.y()) < SAME_DIST)
                // {
                //     if (v1.y() > map_center_.y())
                //     {
                //         if (!number_exists(graph_.neighbors(v1.id()), v2.id()) && !number_exists(graph_.neighbors(v2.id()), v1.id()))
                //             graph_.add_edge(v1.id(), v2.id(), vel_list_[idx]);
                //     }
                //     else
                //     {
    ...
    void SingleReconstruction::build_graph_edges_from_zone_and_line(ListVertices vertices)
    {
        int idx = torch::randint(0, vel_list_.size() - 1, 1).item().toInt();
    ...
    bool SingleReconstruction::is_edge_zone_line(Vertex v1, Vertex v2)
    {
    ...
    void SingleReconstruction::build_graph_edges_from_single_horizontal_line()
    {
        int index = 0;
        for (Vertices list : s_h_vertices_)
        {
            ...
            else if (index == s_h_lines_.size() - 1)
            {
                for (int i = 0; i < v_list.size() - 1; i++)
                {
    ...
    void SingleReconstruction::build_graph_edges_from_single_vertical_line()
    {
        int index = 0;
        for (Vertices list : s_v_vertices_)
        {
            ...
            else if (index == s_v_lines_.size() - 1)
            {
                for (int i = 0; i < v_list.size() - 1; i++)
                {
                    if (!number_exists(graph_.neighbors(v_list[i + 1].id()), v_list[i].id()))
    ...
    void SingleReconstruction::add_line(int type, Point2D center, float length)
    {
    ...
    void SingleReconstruction::add_zone_point(int point_type, int line_type, Point2D center, float length, float width)
    {
    ...
    void SingleReconstruction::calculate_waiting_zone_bridge()
    {
        float distance;
    ...
    void SingleReconstruction::visualize(matplot::axes_handle visual, bool show_edge)
    {
        visual->hold(true);
