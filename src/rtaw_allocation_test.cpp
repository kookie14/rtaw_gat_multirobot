...
int main()
{
    ...
    rds::RTAWBufferHandle test_buffer = std::make_shared<rds::RTAWBuffer>(num_test_task, num_test_robots, num_task_queue, 0.0, 0.0, device);
    ...
    torch::jit::script::Module rtaw_module = torch::jit::load("../data/statistics/rtaw_single/" + env_name + "/train_3/best_average_time_model.pth", device);
    ...
    while (true)
    {
        ...
        if (i == std::max(num_test_robots, 100))
        {
            ...
            fig_test->save("../visualize/env_rtaw_test.png");
        }
        i += 1;
